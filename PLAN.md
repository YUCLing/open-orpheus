...
**Revision:** 29 (2026-09-22) — P2-1 begins: `src/main/calls/` → `src/main/ipc/`, with the
dispatcher facade at `ipc/dispatcher.ts` and the specs mirrored. `src/main/` is down from
46 to 45 direct entries; the domain/platform/services grouping is still to come.
...
| P2-1  | Group `src/main/*` per §5.3                                                                    | P2    | **In progress** | 2026-09-22 | **Done so far:** `src/main/calls/` → `src/main/ipc/`, and the dispatcher facade `calls.ts` → `ipc/dispatcher.ts` (the `calls.ts` + `calls/` pair was exactly the ambiguous facade shape §9.1 describes). Specs mirrored to `__test__/main/ipc/`, with `dispatcher.spec.ts` renamed from `calls.spec.ts`. Non-import references updated per §5.3's checklist: both `CONTRIBUTING.md` and `docs/CONTRIBUTING_en.md` tree diagrams, the `call module barrel` guard spec (which also had to exclude `dispatcher.ts` from its module list, since it now lives beside the call modules), and two stale prose comments. **Remaining:** group the other 45 `src/main/*` entries into `domain/`, `platform/` and `services/` per §5.3, and get every directory under 15 direct entries                                                                   |
