...
**Revision:** 39 (2026-09-22) — P2-5 step 3: the root `lifecycle.ts` is merged into
`services/lifecycle.ts`, so there is one lifecycle module and no near-collision with
the service of the same name. `src/main/` now holds only the six group directories plus
`logger.ts`.
...
| P2-5  | P2-1 placement follow-ups: `services/` locations, `window.ts`, `lifecycle.ts`, `logger.ts`     | P2    | **In progress** | 2026-09-22 | Four decisions the reorganisation surfaced, approved by the repo owner. **(1) done** — `bootstrap/services/*` folded into `src/main/services/`, so `bootstrap/` is purely the composition root and `database.ts`+`database/` finally sit together. **(2) done** — `window.ts` → `windows/managedWindow.ts`. **(3) done** — root `lifecycle.ts` merged into `services/lifecycle.ts`. **(4)** `LoggerPlugin` gains a naming-base option so `logger.ts` can move to `platform/` without renaming a single log record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
