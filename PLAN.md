...
**Revision:** 22 (2026-09-22) — P1-10 continues: `tray.ts` becomes `createTray(deps)`
with no module-scope `Tray`, `showManageWindow(deps)` and `registerTrayicon(deps)`
follow, and `startup.ts` reads the proxy through `ctx.settings.kv`. The window
factories, the audio engines and `main.ts` remain.
