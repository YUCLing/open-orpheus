...
**Revision:** 31 (2026-09-22) — P2-1 continues: the `platform/` group landed (14 files),
specs mirrored, and both CONTRIBUTING trees now describe the groups. `src/main/` is down
from 46 to 25 direct entries. `logger.ts` is deliberately **not** moved — `LoggerPlugin`
injects its import at build time and derives log names from its directory.
...
| P2-1  | Group `src/main/*` per §5.3                                                                    | P2    | **In progress** | 2026-09-22 | **Done so far:** `ipc/` (§5.3-named; facade at `ipc/dispatcher.ts`), `domain/` (§5.3-named: `lyrics.ts`+`lyrics/`, `skin/`, `playback/`, `id3.ts`, `ncae.ts`, `afp.ts`, `crypto.ts`) and `platform/` (§5.3-named six plus `orpheus`, `folders`, `gui`, `update`, `dawn`, `anonymous`, `error`, `util`), with specs mirrored under `__test__/main/{ipc,domain,platform}/`. `src/main/` direct entries: 46 → 25. **Three gaps found in §5.3, recorded here rather than silently patched:** (1) it names only 21 of the 45 entries, but this task's exit criterion is "no directory with >15 direct entries", so the plan cannot satisfy itself as written — I am applying its own stated principle (stateful + injectable → `services/`; OS/app integration → `platform/`; `BrowserWindow` work → `windows/`) and recording each call; (2) **`ipc/` cannot meet the ≤15 rule either** — with `channel.ts` added it holds 17 files (15 handlers + `index.ts` + `dispatcher.ts`), so its handlers need an `ipc/handlers/` level; (3) it lists "window" under `services/` while keeping a separate `windows/`, and calls `domain/` "pure logic" while placing the stateful `playback/` and `lyrics/` there. **Also pinned:** `src/main/logger.ts` must not move — `build-plugins/LoggerPlugin.ts` injects its import at build time and derives every module's log `name` from the logger's directory, so moving it both breaks the build and changes log output. **Remaining:** `services/` and the §5.3-unnamed bucket, then the `ipc/handlers/` split |
