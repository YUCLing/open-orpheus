...
**Revision:** 42 (2026-09-22) — `ipc/` reverted to `calls/`. §5.3's rename contradicted
§3.1: the `CallDispatcher` is the web pack's ABI reached through `channel.call`, not this
project's IPC, which is `src/bridge/**`. Naming it `ipc/` collided with the real one.
