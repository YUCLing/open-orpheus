...
**Revision:** 30 (2026-09-22) — P2-1 continues: the `domain/` group landed
(`lyrics`, `skin`, `playback`, `id3`, `ncae`, `afp`, `crypto`), with specs mirrored.
`src/main/` is down from 46 to 38 direct entries; `platform/` and `services/` remain.
...
| P2-1  | Group `src/main/*` per §5.3                                                                    | P2    | **In progress** | 2026-09-22 | **Done so far:** `ipc/` (§5.3-named; facade at `ipc/dispatcher.ts`), and `domain/` (§5.3-named: `lyrics.ts`+`lyrics/`, `skin/`, `playback/`, `id3.ts`, `ncae.ts`, `afp.ts`, `crypto.ts`), with every spec mirrored under `__test__/main/{ipc,domain}/`. `src/main/` direct entries: 46 → 38. **A gap worth recording:** §5.3 names only 21 of the 45 entries, but this task's exit criterion is "no directory with >15 direct entries", so the plan cannot satisfy itself as written — the remaining ~24 files need assignment. Where §5.3 is silent I am applying its own stated principle (stateful + injectable → `services/`; OS/app integration → `platform/`; `BrowserWindow` work → `windows/`) and recording each call. §5.3 also conflicts with itself twice: it lists "window" under `services/` while keeping a separate `windows/`, and describes `domain/` as "pure logic" while placing the stateful `playback/` and `lyrics/` there. **Remaining:** `platform/`, `services/`, and the §5.3-unnamed bucket, to reach ≤15 entries per directory |
