...
**Revision:** 32 (2026-09-22) — P2-1 continues: the `services/` group landed (12 entries),
specs mirrored, doc trees updated. `src/main/` is down from 46 to **14** direct entries.
`window.ts` and `lifecycle.ts` stay at the root for now — see the ledger for why.
...
| P2-1  | Group `src/main/*` per §5.3                                                                    | P2    | **In progress** | 2026-09-22 | **Done so far:** `ipc/`, `domain/`, `platform/` (§5.3-named plus the OS/app bucket) and `services/` (`audio*`, `av3a/`, `cache*`, `database/`, `download`, `mediaSession`, `pack*`, `packs/`, `tray`), specs mirrored under `__test__/main/{ipc,domain,platform,services}/`. `src/main/` direct entries: 46 → 14. **Four gaps found in §5.3, recorded rather than silently patched:** (1) it names only 21 of the 45 entries, yet this task's exit criterion is "no directory with >15 direct entries", so the plan cannot satisfy itself as written — I apply its own principle (stateful + injectable → `services/`; OS/app integration → `platform/`; `BrowserWindow` work → `windows/`) and record each call; (2) **`ipc/` cannot meet the ≤15 rule** — with `channel.ts` it holds 17 files (15 handlers + `index.ts` + `dispatcher.ts`), so handlers need an `ipc/handlers/` level; (3) it lists "window" under `services/` while keeping a separate `windows/`, and calls `domain/` "pure logic" while placing the stateful `playback/` and `lyrics/` there; (4) **§5.1 and §5.3 disagree about where services live** — §5.1's sketch puts them at `bootstrap/services/` (what P1 built) and §5.3's target tree puts `services/` beside `bootstrap/`. I followed §5.3 for the newly moved ones and left P1's `bootstrap/services/` in place, so there are currently two services locations; consolidating them is a decision, not a mechanical move. **Two files pinned at the root for now:** `logger.ts` (moving it breaks `LoggerPlugin`'s build-time injected import _and_ renames every log record) and `window.ts` (moving it into `services/` would collide with the existing `bootstrap/services/window.ts`). **Remaining:** the reassignments (`channel.ts` → `ipc/`, `menu*` → `windows/`, `startup.ts`+`process-setup.ts` → `bootstrap/`), the `ipc/handlers/` split, the `window.ts`/`lifecycle.ts` decisions, and the two-services-location consolidation |
