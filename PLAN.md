...
**Revision:** 48 (2026-09-22) — P3-4(c): `exactOptionalPropertyTypes` started (51 → 29) and
assessed. It is tractable but broad, and satisfying it means widening our own optional
declarations. The mutable-accumulator fixes are landed; the rest is scoped in the ledger.
...
| `exactOptionalPropertyTypes`                      | ~~47~~ **51**                          | Cheap-ish; needs care (unrelated to `strict`). **[V]**                                                                                                                                                                                                                                                              |
...
| P3-4  | Mid-cost strictness flags (130 errors total)                                                   | P3    | **In progress** | 2026-09-22 | **`noPropertyAccessFromIndexSignature` enabled** (35 sites, applied by driving `tsc`'s own suggestions). **`verbatimModuleSyntax` blocked** — 47 mechanical sites plus 5 ambient const enums the compiler cannot inline. **`exactOptionalPropertyTypes`: started, 51 → 29.** The two mutable-accumulator fixes landed (`Partial<MiniPlayerStyle>` and `ThumbnailOptions` now permit explicit `undefined`, which is what the code already stores). The remaining 29 are all one class — our own optional properties receiving explicit `undefined` — and split into ~25 declaration widenings (an optional property widened to also permit explicit `undefined`) plus 4 caller-side fixes where the target is third-party: the DOM's `CustomEventInit`, `got`'s `ExtendOptions`, `cookie`'s `SetCookie`, and one Electron overload. **Judgement for the reviewer:** the widenings are behaviour-preserving and accurate, but they also erase the distinction the flag exists to enforce; the alternative — omitting keys at call sites — changes object shapes (spread, `Object.keys`, `in`), which P3's no-behaviour-change bar rules out. So the flag is attainable, at the cost of ~19 declarations on which it would then have nothing left to check.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
