...
**Revision:** 21 (2026-09-22) — P1-10 continues: `createMediaSession(deps)` takes the
window accessor, so `mediaSession.ts` no longer imports it. `tray.ts`, the window
factories, the audio engines, `audio.ts` and `main.ts` remain.
...
| P1-10 | Delete the legacy adapter                                                                      | P1    | **In progress** | 2026-09-22 | Adapter still present, but every call module takes its dependencies and the non-call consumers are being converted: `createMediaSession(deps)` now takes the window accessor. Injected: `audioeffect` (`readEffect`), `browser` (cookie API), `app` (`settings.kv`, `lifecycle.setLifecycleState`), `musiclibrary` (`musicLibraryDb`), `storage` (`webDb`, window accessor), `winhelper` (`settings`, `lifecycle`, window-typed accessor), `mediaSession` (window accessor). `startApplication()` keeps the `ReadyPhase` and calls `registerCallModules(deps)`, so `channel.ts` no longer triggers the barrel. **Remaining:** `tray.ts` (creates a `Tray` at module scope and is consumed by `calls/trayicon.ts`), `windows/{main,manage,desktop-lyrics,mini-player}.ts`, `audio.ts`, `audio/MediaEngine.ts`, `av3a/Av3aEngine.ts`, `main.ts`, and `main/lyrics.ts` — which needs `playbackController` injected before its spec's mock can go |
