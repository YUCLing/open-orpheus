...
**Status:** in execution. P0 is complete; P1 is in progress, with P1-10 remaining
before P2. Design sections below still carry their evidence tags.
**Revision:** 25 (2026-09-22) — P1-10 continues: `audio.ts` becomes `createAudio(deps)`
and the two engines take the window service by constructor. Every adapter binding except
`mainWindow` now has zero production consumers; `main.ts` and `main/lyrics.ts` remain.
...
| --------------------------------- | ---------------------- | ------------------------------------ | ----- |
| Own-module `vi.mock` sites        | 11 in 10 files **[V]** | ~~0~~ **1** after P1-10a; 0 in P1-10 | P1    |
...
| P1-10 | Delete the legacy adapter                                                                      | P1    | **In progress** | 2026-09-22 | Adapter still present, but shrinking. Injected: every `calls/*.ts` module (`audioeffect`, `browser`, `app`, `musiclibrary`, `storage`, `winhelper`, `trayicon`), plus `mediaSession` and `windows/manage`. `tray.ts` is now `createTray(deps)` returning a `TrayService`, so importing it no longer constructs a `Tray`; `startApplication()` creates it and passes it to `registerCallModules(deps)`. `startup.ts` reads the proxy through `ctx.settings.kv` instead of importing the adapter. **Remaining:** `main.ts` — the last `mainWindow` consumer — and `main/lyrics.ts`, which needs `playbackController` injected before its spec's mock can go. Everything else is now unconsumed: `currentState`, `setLifecycleState`, `webDb`, `musicLibraryDb`, `nativeDb`, `kv`, `events` (settings) and `mainWindowAccessor` have no production importers, so the bindings can be deleted once `main.ts` is converted |
| P1-11 | Give each `calls/*.ts` a `register()` function                                                 | P1    | **Done**        | 2026-09-22 | Measured scale was **133** registrations across 14 modules — not the ~470 first estimated. Every top-level expression statement in the directory (the 133 registrations plus `winhelper`'s import-time settings subscription) moved into an exported `register()`; module-level types/state/helpers stayed put, so registration order is unchanged. `index.ts` exports `registerCallModules()`, which `channel.ts` now calls explicitly. The guard spec pins "no top-level registration" and "every module exports register()", both falsified. The last 3 mocks are _unblocked_, not removed — that is P1-10                                                                                                                                                                                                                                                                                                         |
...
- Own-module `vi.mock` sites: ~~**11 → 0**~~ **11 → 3 in P1-9**, with 0 in P1-10.
  This is the metric that proves the refactor addressed the actual problem. The last
  three mock `@main/audio`, `@main/cookie` and `@main/mediaSession` in specs for
  `calls/{audioeffect,browser}.ts` and `main/lyrics.ts`, all of which registered or
  subscribed at **import time** over a closed-over import, so there was no seam to
  inject through. P1-11 supplied the seam for the two `calls/*.ts` modules; deleting
  those mocks means taking the dependencies as parameters, which is P1-10's job. See
