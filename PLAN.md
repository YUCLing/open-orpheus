...
**Revision:** 20 (2026-09-22) — P1-10 continues: `WindowService` gains a
window-typed `currentWindow()`, which brings `calls/winhelper.ts` off the ambient
`mainWindow`/`settings`/lifecycle imports — every call module now takes its
dependencies. The non-call consumers remain, so P1-10 stays in progress.
...
| P1-10 | Delete the legacy adapter                                                                      | P1    | **In progress** | 2026-09-22 | Adapter still present, but every call module now takes its dependencies: `audioeffect` (`readEffect`), `browser` (cookie API), `app` (`settings.kv`, `lifecycle.setLifecycleState`), `musiclibrary` (`musicLibraryDb`), `storage` (`webDb`, window accessor) and `winhelper` (`settings`, `lifecycle`, window-typed accessor). `startApplication()` keeps the `ReadyPhase` and calls `registerCallModules(deps)`, so `channel.ts` no longer triggers the barrel. **Remaining:** the non-call consumers — `tray.ts`, `audio.ts`, `mediaSession.ts`, `audio/MediaEngine.ts`, `av3a/Av3aEngine.ts`, `windows/*`, `main.ts` — plus `main/lyrics.ts`, which still needs `playbackController` injected before its spec's mock can go |
