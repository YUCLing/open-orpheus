...
**Revision:** 18 (2026-09-22) — P1-11: every `calls/*.ts` exports `register()` instead
of registering on import (133 registrations, 14 modules), the barrel invokes them
explicitly, and the guard test pins both invariants. The `vi.mock` metric is corrected
again: P1-11 supplies the seam but deletes no mock, so 0 lands in P1-10.
...
| --------------------------------- | ---------------------- | ---------------------------------- | ----- |
| Own-module `vi.mock` sites        | 11 in 10 files **[V]** | ~~0~~ **3** after P1-9; 0 in P1-10 | P1    |
