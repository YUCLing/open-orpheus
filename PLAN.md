...
| --------------------------------- | ---------------------- | -------------------------------- | ----- |
| Own-module `vi.mock` sites        | 11 in 10 files **[V]** | ~~0~~ **3** after P1; 0 at P1-11 | P1    |
...
| P1-11 | Give each `calls/*.ts` a `register()` function                                                 | P1    | Not started  |            | Now the **only** route to P1-6's goal, since the table was reverted. ~470 `registerCallHandler` statements across 15 files, interleaved with module-level types/state/helpers — **codemod-scale, not hand edits**, and it sits on the call ABI (§3.1). It is also what unblocks the last 5 `mainWindow` consumers, since they could then take injected deps. Do not attempt piecemeal                                                    |
...
- Own-module `vi.mock` sites: ~~**11 → 0**~~ **11 → 3 in P1**, with 0 at P1-11.
  This is the metric that proves the refactor addressed the actual problem. The last
  three are not reachable from P1: they mock `@main/audio`, `@main/cookie` and
  `@main/mediaSession` in specs for `calls/{audioeffect,browser}.ts` and
  `main/lyrics.ts`, all of which register or subscribe at **import time** over a
  closed-over import. A module that is only ever imported for its side effect has no
  seam to inject through — that is precisely what P1-11 provides. See §10 for the
  verification.
