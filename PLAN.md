...
**Revision:** 41 (2026-09-22) — `services/` restored to 15 entries: step 1 had pushed it
to 16, over P2-1's own rule. `av3a/` now sits under `services/audio/`, where it belongs.
`logger.ts`.
...
| P2-1  | Group `src/main/*` per §5.3                                                                    | P2    | **Done**     | 2026-09-22 | Reorganised into `bootstrap/`, `services/`, `domain/`, `platform/`, `ipc/` (+ `ipc/handlers/`) and `windows/` (+ `windows/menu/`). `src/main/` direct entries: **46 → 9**, and **no directory exceeds 15** (bootstrap 5, domain 8, ipc 4, ipc/handlers 15, platform 14, services 12, windows 7, windows/menu 4) — the exit criterion is met. **Corrected 2026-09-22:** the "no behaviour change" claim was wrong for log output — renaming `calls/` silently dropped the main process's per-command `call` loggers; see P2-5(4) and §10. Specs mirror every group. **Four §5.3 gaps found and worked around rather than hidden:** it names only 21 of the 45 entries; `ipc/` would hold 17 once `channel.ts` joined, so handlers moved to `ipc/handlers/`; it lists "window" under `services/` while keeping `windows/`, and calls `domain/` "pure logic" while placing the stateful `playback/` and `lyrics/` there; and §5.1 vs §5.3 disagree on where services live (§5.1's `bootstrap/services/` is what P1 built, §5.3's tree puts `services/` beside `bootstrap/`) — the newly moved ones followed §5.3, so **two services locations coexist**. **Three files deliberately left at the root:** `logger.ts` (moving it breaks `LoggerPlugin`'s build-time injected import _and_ renames every log record), `window.ts` (would collide with `bootstrap/services/window.ts`) and `lifecycle.ts` (would collide with `bootstrap/services/lifecycle.ts`). **Open follow-up, not a mechanical move:** consolidating the two services locations and placing those three files. |
