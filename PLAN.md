...
**Revision:** 19 (2026-09-22) — P1-10 started: five `calls/*.ts` modules take their
dependencies as parameters, and `startApplication()` wires them from the `ReadyPhase`
it previously discarded. Own-module mocks 3 → 1. `winhelper` and the non-call adapter
consumers remain, so P1-10 stays in progress.
...
| P1-10 | Delete the legacy adapter                                                                      | P1    | **In progress** | 2026-09-22 | Adapter still present, but its consumer set is shrinking. Injected so far: `calls/audioeffect.ts` (`readEffect`), `calls/browser.ts` (cookie API), `calls/app.ts` (`settings.kv`, `lifecycle.setLifecycleState`), `calls/musiclibrary.ts` (`musicLibraryDb`) and `calls/storage.ts` (`webDb`, window accessor). `startApplication()` keeps the `ReadyPhase` and calls `registerCallModules(deps)`, so `channel.ts` no longer triggers the barrel. **Remaining:** `calls/winhelper.ts` — which needs a window accessor exposing the `BrowserWindow`, not just `RendererTarget` — plus `tray.ts`, `audio.ts`, `mediaSession.ts`, `audio/MediaEngine.ts`, `av3a/Av3aEngine.ts`, `windows/*`, `main.ts`, and `main/lyrics.ts` (the last mock) |
