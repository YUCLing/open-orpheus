...
**Status:** in execution. P0 is complete; P1 is nearly complete, with P1-12 remaining
before P2. Design sections below still carry their evidence tags.
**Revision:** 26 (2026-09-22) — **P1-10 done: the legacy adapter is deleted.**
`src/main/{settings,database}.ts` are gone, `window.ts` no longer re-exports the window
bindings, and `main.ts` reads the window service directly. `lifecycle.ts` keeps
`currentState`/`setLifecycleState` for the pre-`bootstrap()` entry point; the last mock
is now its own task, P1-12.
...
| --------------------------------- | ---------------------- | ----------------------------------- | ----- |
| Own-module `vi.mock` sites        | 11 in 10 files **[V]** | ~~0~~ **1** after P1-10; 0 at P1-12 | P1    |
...
| P1-12 | Inject `playbackController` into `main/lyrics.ts`; drop the last own-module mock               | P1    | Not started  |            | `lyrics.spec` mocks `@main/mediaSession` because `main/lyrics.ts` subscribes to `playbackController` at import time. Not mechanical: `lyricsDispatcher` is imported at module scope by both on-demand window factories, and `mediaSession` (which owns `playbackController`) loads concurrently with them in `startApplication`'s `Promise.all` — so the creation order has to be settled before the injection can be written                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- Own-module `vi.mock` sites: ~~**11 → 0**~~ **11 → 3 in P1-9**, **1 after P1-10**,
  0 at P1-12.
  This is the metric that proves the refactor addressed the actual problem. Three of
  the originals mocked `@main/audio`, `@main/cookie` and `@main/mediaSession` in specs
  for `calls/{audioeffect,browser}.ts` and `main/lyrics.ts`, all of which registered or
  subscribed at **import time** over a closed-over import, so there was no seam to
  inject through. P1-11 supplied the seam; P1-10 took the dependencies as parameters
  and deleted the first two. The survivor is `main/lyrics.ts`, whose
  `playbackController` injection has an ordering dependency on `mediaSession` — P1-12.
  See §10 for the verification.
