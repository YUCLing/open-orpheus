...
**Revision:** 16 (2026-09-21) — P1-7: the entry is 73 lines; the pack/commit loop is
injectable and tested. §5.1 corrected: `bootstrap()` does not own the app-level sequence.
...
| 2026-09-21 | P1-7: the entry is now 73 lines **[V]**                                                                                                                                                                                     | `src/main.ts` went 483 → 73. Process configuration moved to `src/main/process-setup.ts` (`configureProcess()`), the ordered start-up sequence to `src/main/startup.ts` (`startApplication()`), and the 105-line pack/commit decision loop to `src/main/pack-loader.ts`. `grep -E 'while                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | for \( | await import | Promise\.all | readFile | writeFile' src/main.ts`finds nothing, so the §7 exit criterion "no sequencing loops or multi-step init chains" holds.`pnpm test`60 files / 524 tests,`pnpm lint`clean,`pnpm package` succeeded. |
