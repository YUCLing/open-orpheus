...
**Revision:** 24 (2026-09-22) — P1-10 continues: both on-demand window factories take
`deps`, stored on the `OnDemandWindow` instance so per-`show()` recreations still see
it. The audio engines, `audio.ts`, `main.ts` and `main/lyrics.ts` remain.
